...
func (p *ControllerRegister) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	if !isRunnable {
		...
		resp = execController.GetResponseData()
		requestBody = string(execController.GetRequestBody())
	...
	if true {
		...
		if findRouter {
			if routerInfo != nil {
				...
				//					routerInfo.pattern)
				devInfo = fmt.Sprintf("|%s %3d %s|%13s|%8s|%s %s %-7s %-3s %s  r:%s | %s | %s | %s | %v", statusColor, statusCode,
					resetColor, timeDur.String(), "match", methodColor, resetColor, r.Method, realIp, r.URL.Path,
					routerInfo.pattern, r.URL.String(), r.Form.Encode(), requestBody, resp)
			} else {
				...
				devInfo = fmt.Sprintf("|%s %3d %s|%13s|%8s|%s %s %-7s %s %-3s| %s | %s | %s | %v", statusColor, statusCode, resetColor,
					timeDur.String(), "match", methodColor, resetColor, r.Method, realIp, r.URL.Path, r.URL.String(), r.Form.Encode(), requestBody, resp)
			}
		} else {
			//			devInfo = fmt.Sprintf("|%s %3d %s|%13s|%8s|%s %s %-7s %-3s", statusColor, statusCode, resetColor,
			//				timeDur.String(), "nomatch", methodColor, resetColor, r.Method, r.URL.Path)
			devInfo = fmt.Sprintf("|%s %3d %s|%13s|%8s|%s %s %-7s %s %-3s| %s | %s | %s | %v", statusColor, statusCode, resetColor,
				timeDur.String(), "nomatch", methodColor, resetColor, r.Method, realIp, r.URL.Path, r.URL.String(), r.Form.Encode(), requestBody, resp)
		}
		if iswin {
